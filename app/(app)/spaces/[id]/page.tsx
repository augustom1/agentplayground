...
export default function SpacePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Filter to only teams in this playground
      const filtered = pg.teamIds.length > 0
        ? allTeams.filter(t => pg.teamIds.includes(t.id))
        : [];

      // Each team from /api/teams has _count but not agents/skills arrays — fetch agents
      // The existing /api/teams returns teams with _count. We need agents separately.
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0 }}>
          ...
          {editing ? (
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--color-text)", margin: 0 }}>
              {playground.icon ? "" : ""}{playground.name}
            </h1>
          )}
        </div>
