...
- **Customizable dashboards + menus** (✅ first cut Session 37). Widget registry formalized (add/remove/reorder/persist, per-user for Overview, per-playground for playground dashboards); customizable playground inner menu with **Schedule as a standard item (per-playground scoped)**. Schema: layout JSON on `Playground` + a user-level dashboard config.
  - **Follow-up (owner feedback 2026-07-07, → Session 39):** (a) Move the Overview hub's section switcher (Dashboard/Brain/Schedule/Optimize/Websites/Tools) from the top tab bar into the **left sidebar** when the Overview tab is active — *"options on the right should be on the left, like the chat window."* (b) Let users build **custom dashboards from agent-team data** — extend `lib/widget-registry.ts` beyond the fixed 5 Overview widgets to per-team / per-agent widgets (a team's agents/tasks/completions/skills). Pairs with the team drill-down (item 5).
- **Team drill-down + agent editor** (🔜 NEXT — Session 39, pulled forward by owner feedback 2026-07-07; was deferred). Owner tested Session 38 and hit a **crash: the TEAMS list inside a playground links to a route that doesn't exist → hard 404.**
  - **Path mismatch to reconcile first:** `app/(app)/playground/[id]/layout.tsx:189` links to `/playground/[id]/team/[teamId]` (**singular `team`**), but this plan previously said `/playground/[id]/teams/[teamId]` (plural). Pick ONE and make the link + the new route directory match (slug rule: `[teamId]`).
  - **Team page** `app/(app)/playground/[id]/team/[teamId]/page.tsx`: list the team's agents. Owner wants a **markedly better, more customizable interface — build it with the `ui-ux-pro-max` skill.**
  - **Agent popup (owner's core ask):** click an agent → window shows its **preferred/required model**. `Agent.model` already exists (default `claude-sonnet-4-6`); edit it via `PATCH /api/agents/[id]` with the model catalog (`components/ModelPicker.tsx` + `lib/model-catalog.ts`). Add a coordinator chat tool so the **coordinator can set an agent's required model**; if unspecified the agent uses the predetermined default. Later: skills allowlist, Brain doc grants, file access (per-agent capability records — groundwork for permission rings §5).
  - **Create-a-team button (owner ask):** add a simple create-team UI inside the playground mirroring the create-playground flow (`POST /api/teams` exists → attach to the playground's `teamIds`). Must work for a non-technical user **without** the coordinator.
  - **Theme:** simpler, low-computer-expertise UI; agents accessible + easy to manage.
