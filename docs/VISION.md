...
> **UPDATE 2026-07-04 (owner feedback — overrides §2.1 where they conflict):** After reviewing the deployed UI, the owner redefined the target. The shell follows the **Claude Desktop pattern** — sidebar tab pill with exactly three tabs, **Chat | Playgrounds | Overview** (no top-level Teams or Brain; teams live inside playgrounds), centered home greeting + large coordinator input. The **Overview tab is the system hub**: the customizable widget dashboard (widgets from playgrounds, teams, total tasks — whatever the user wants in one window) + the full Brain window + Optimize, websites, and other previously-cut utilities. The Chat tab carries a **Brain access point** that redirects to the Brain window in Overview. Inside a playground, Brain shows only what's shared with that playground. The Playgrounds tab holds a coordinator quick-chat task router (with team-confirmation popup + manual override) and the playground list. Playground dashboards and their inner menus become user-customizable; teams drill down to a per-agent editor (model, skills, Brain doc access, file permissions). The Chat tab also carries **Projects** — isolated, disposable multi-playground workspaces (own folder, own Brain namespace, teams by reference, one-tap zero-residue teardown) where multi-team shared-file work happens. Overview additionally holds the **global Schedule**; each playground has its own scoped schedule (same pattern as Brain). Theme: dark = charcoal/grey + blue accents (default), light = white/grey + the same blue. Logo: original pixel-art playground equipment (never Anthropic-adjacent). Full spec + session breakdown: `docs/PLAN.md` §1. Everything in §2.2 (visual rules, no emojis) still applies.
