...
export const CHAT_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Create a pending action item that requires user attention or a manual step. This shows as a ! badge in the sidebar and on the Actions page. Use this when you identify something the user needs to provide, decide, or do manually — e.g. 'Need your ARQ account address', 'Review and approve pricing tier', 'Add work experience to CV'. Be specific and actionable.",
    input_schema: {
      type: "object",
      properties: {
        title: { type: "string", description: "Short title (max 80 chars) — what needs to be done" },
        description: { type: "string", description: "Full description of what is needed and why, with context for the user" },
        category: {
          type: "string",
          enum: ["business", "cv", "education", "technical", "general"],
          description: "Category for grouping and routing to the right team",
        },
        priority: {
          type: "string",
        ...
        context: {
          type: "object",
          description: "Optional JSON context the coordinator wants to attach (e.g. which doc this relates to)",
        },
      },
      required: ["title", "description"],
    },
  ...
  {
    name: "dismiss_pending_action",
    description:
      "Mark a pending action as done or snoozed. Call this when the user has resolved an action item (provided the info, approved the plan, completed the task). Pass the action ID which you received when creating it.",
    input_schema: {
      type: "object",
      properties: {
        actionId: { type: "string", description: "ID of the pending action to update" },
        status: { type: "string", enum: ["done", "snoozed"], description: "Mark as done (resolved) or snoozed (defer for later)" },
      },
  ...
  },
  {
    ...
    description:
      "List all open pending action items. Use this at the start of a conversation to check what the user still needs to provide or decide. Also use it to find action IDs before dismissing them.",
    input_schema: {
      type: "object",
      properties: {
        status: { type: "string", enum: ["open", "snoozed", "done", "all"], description: "Filter by status (default: open)" },
      },
    },
  },
  {
  ...
  try {
    switch (toolName) {
      ...
      case "create_pending_action":
        return await toolCreatePendingAction(input);
      case "dismiss_pending_action":
        return await toolDismissPendingAction(input);
      case "list_pending_actions":
        return await toolListPendingActions(input);
      default:
...
async function toolCreatePendingAction(input: Record<string, unknown>): Promise<string> {
  const title = (input.title as string)?.trim();
  const description = (input.description as string)?.trim();
  if (!title || !description) return JSON.stringify({ error: "title and description required" });
  ...
  return JSON.stringify({
    success: true,
    actionId: action.id,
    message: `✓ Action item created: "${action.title}" (${action.priority} priority, ${action.category}). Shown as ! in sidebar.`,
  });
}

async function toolDismissPendingAction(input: Record<string, unknown>): Promise<string> {
  const actionId = (input.actionId as string)?.trim();
  const status = (input.status as string);

  if (!actionId || !["done", "snoozed"].includes(status)) {
    return JSON.stringify({ error: "actionId and status (done|snoozed) required" });
  }

  await prisma.pendingAction.update({ where: { id: actionId }, data: { status } });

  return JSON.stringify({ success: true, message: `✓ Action ${actionId} marked as ${status}.` });
}

async function toolListPendingActions(input: Record<string, unknown>): Promise<string> {
  ...
  if (actions.length === 0) {
    return JSON.stringify({ count: 0, message: `No ${status} action items found.`, actions: [] });
  }

  return JSON.stringify({
    count: actions.length,
    message: `Found ${actions.length} ${status} action item(s).`,
